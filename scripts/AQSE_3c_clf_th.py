...
class ModelTrainer(ABC):
    ...
    def train(self, train_df: pd.DataFrame, test_df: pd.DataFrame, 
              ...
              threshold: Optional[str] = None, **kwargs) -> Dict[str, Any]:
        """
...
class ChempropTrainer(ModelTrainer):
    ...
    def train(self, train_df: pd.DataFrame, test_df: pd.DataFrame, 
        ...
        try:
            ...
            ## check
            # Apply oversampling to address class imbalance (similar to RF's balanced_subsample)
            from collections import Counter
            from sklearn.utils import resample
            
            class_counts_before = Counter(y_train_split)
            self.logger.info(f"Class distribution before oversampling: {dict(class_counts_before)}")
            
            # Determine target class size (use median class size to avoid extreme oversampling)
            class_sizes = list(class_counts_before.values())
            if len(class_sizes) > 1:
                target_size = int(np.median(class_sizes))
                # But don't oversample too much - cap at 2x the majority class
                max_size = max(class_sizes)
                target_size = min(target_size * 2, max_size * 2)
                ...
                for class_label in range(self.n_classes):
                    ...
                    class_mask = y_train_split == class_label
                    ...
                    train_smiles_resampled.extend([train_smiles_split[i] for i in class_indices])
                    train_features_resampled.append(train_features_split[class_indices])
                    y_train_resampled.extend([y_train_split[i] for i in class_indices])
                    
                    # If this is a minority class, oversample it
                    if len(class_indices) < target_size:
                        n_samples_needed = target_size - len(class_indices)
                        # Sample with replacement (set seed for reproducibility)
                        ...
                        train_smiles_resampled.extend([train_smiles_split[i] for i in oversampled_indices])
                        train_features_resampled.append(train_features_split[oversampled_indices])
                        y_train_resampled.extend([y_train_split[i] for i in oversampled_indices])
                
                # Rebuild arrays
                train_smiles_split = train_smiles_resampled
                train_features_split = np.vstack(train_features_resampled)
                y_train_split = np.array(y_train_resampled, dtype=np.int64)
                
                class_counts_after = Counter(y_train_split)
                self.logger.info(f"Class distribution after oversampling: {dict(class_counts_after)}")
                self.logger.info(f"Training samples: {len(train_smiles_split)} (was {len(train_smiles_valid)})")
            else:
                self.logger.info("Skipping oversampling - only one class present")
            ...
            self.logger.warning("  - Manual oversampling of minority classes in data preparation")
...
class BioactivityDataLoader:
    ...
    def calculate_physicochemical_descriptors(self, smiles: str) -> Dict[str, float]:
        ...
        """
        try:
            # Import the physicochemical descriptors module
            import sys
            from pathlib import Path
            
            # Try to find analyses directory
            analyses_dir = None
            
            # Check environment variable first
            if os.getenv('ANALYSES_DIR'):
                analyses_dir = Path(os.getenv('ANALYSES_DIR')).expanduser().resolve()
            else:
                # Try relative paths from script location
                script_dir = Path(__file__).parent.absolute()
                possible_paths = [
                    script_dir.parent.parent / "analyses",
                    script_dir.parent / "analyses",
                    script_dir / "analyses",
                ]
                for path in possible_paths:
                    if path.exists() and (path / "physicochemical_descriptors.py").exists():
                        analyses_dir = path
                        break
            
            if analyses_dir and analyses_dir.exists():
                if str(analyses_dir) not in sys.path:
                    sys.path.insert(0, str(analyses_dir))
                self.logger.debug(f"Using analyses directory: {analyses_dir}")
            else:
                # Try direct import (if installed as package)
                self.logger.debug("Analyses directory not found, trying direct import")
...
class ActivityThresholdsLoader:
    """Handles activity thresholds data loading and processing"""
    def __init__(self, config):
        self.config = config
        self.logger = logging.getLogger(self.__class__.__name__)
        
        # Load activity thresholds
    ...
    def _load_thresholds(self):
        ...
        try:
            self.logger.info(f"Loading activity thresholds from {filepath}")
            df = pd.read_csv(filepath)
    ...
    def get_thresholds(self, uniprot_id: str) -> Dict[str, float]:
        """
        ...
        Returns:
            Dictionary with 'high' and 'medium' cutoff values, or empty dict if not found
        """
        if self.thresholds_df.empty:
            return {}
        
        try:
            # Look up by UniProt ID (assuming it's the index)
            ...
            row = protein.iloc[0]
            return {
                'high': float(row['cutoff_high']),
                'medium': float(row['cutoff_medium'])
            }
        except Exception as e:
            self.logger.error(f"Error getting thresholds for {uniprot_id}: {e}")
            return {}
    
    def get_thresholds_by_name(self, protein_name: str) -> Dict[str, float]:
        """
...
class AQSE3CWorkflow:
    ...
    def train_model_a(self, uniprot_id: str, protein_name: str, thresholds: Dict[str, float]) -> Dict[str, Any]:
        ...
        if self.model_trainer.requires_precomputed_features():
            ...
            train_df, test_df = self.split_data_stratified(features_df, test_size=0.2)
            ...
            bioactivity_data_clean = bioactivity_data[['SMILES', 'class']].copy()
            
            # Split data: 80% train, 20% test with stratification
            train_df, test_df = self.split_data_stratified(bioactivity_data_clean, test_size=0.2)
        ...
        train_results = self._train_model_with_reporting(
            train_df, test_df, model_type='A', protein_name=protein_name, uniprot_id=uniprot_id
        )
    ...
    def assign_class_labels(self, df: pd.DataFrame, thresholds: Dict[str, float]) -> pd.DataFrame:
        """
        Assign 3-class labels to bioactivity data based on pchembl_value_Mean
        
        Args:
            df: DataFrame with pchembl_value_Mean column
            thresholds: Dictionary with 'high' and 'medium' cutoff values
    ...
    def split_data_stratified(self, df: pd.DataFrame, test_size: float = 0.2) -> Tuple[pd.DataFrame, pd.DataFrame]:
        """
...
class ModelReporter:
    ...
    def calculate_metrics(self, y_true: List[int], y_pred: List[int], y_prob: Optional[np.ndarray] = None) -> Dict[str, Any]:
        ...
        # Class labels
        metrics['class_labels'] = ['Low', 'Medium', 'High']
    ...
    def save_predictions(self, df: pd.DataFrame, y_pred: List[int], protein_name: str, uniprot_id: str, model_type: str, 
                        y_prob: Optional[np.ndarray] = None, threshold: Optional[str] = None, split: str = 'test') -> str:
        ...
        try:
            ...
            predictions_df['predicted_class'] = [['Low', 'Medium', 'High'][i] for i in y_pred]
            predictions_df['predicted_class_int'] = y_pred
            
            # Add probabilities if available
            if y_prob is not None:
                predictions_df['prob_low'] = y_prob[:, 0]
                predictions_df['prob_medium'] = y_prob[:, 1]
                predictions_df['prob_high'] = y_prob[:, 2]
